...
def manage_wlan_on_airos(host, username, password, ssid):
    try:
        ...
        commands = [
            ...
            f"config wlan mac-filtering disable {wlan_id}",
            f"config wlan nac radius disable {wlan_id}",
            f"config wlan aaa-override disable {wlan_id}",
            f"config wlan mobility anchor delete {wlan_id} 162.41.34.21",
            f"config wlan mobility anchor add {wlan_id}  162.41.34.23",
